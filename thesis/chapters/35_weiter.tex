...
Zum Einen wird eine Aufteilung der Daten zu einem bestimmten Gewicht (weight) vorgenommen, wobei das Modell auf die vergangenen Daten kalibriert und 
dann auf die zukünftigen Daten angewendet und die Prognosegüte gemessen wird. Über alle Gewichte wird das Mittel (Mean) angegeben.
Zum Anderen wird zu einem festen Zeitinterval (20 Tage) ein sequenzieller Backtest durchgeführt. Alle vergangenen Daten ergeben jeweils eine Prognose.

\paragraph{Interpretation der Ergebnisse}
Beim DAX verhalten sich GBM und CEV sehr ähnlich, wobei die sich Prognose selbstverständlich bei höheren Gewichten verbessert. Das passt zur Parameterschätzung,
da $\beta$ nahe $1$ ist (s.o.) und sich das CEV-Modell dann wie eine GBM verhält. Die Ergebnisse bei der Lufthansa-Aktie sind ebenfalls vergleichbar, aber schlechter, da der
Kurs keinen (log-)linearen Trend aufweist, wie der DAX. Beispielhaft für eine starke Korrelation von Preis und Volatilität (da ist das CEV-Modell vorteilhaft)
wird der Wechselkurs von der türkischen Lira mit dem Dollar während einer Finanzkrise (2021) untersucht (Siehe Abbildung \ref{fig:cev_try}). Dort weist
das CEV-Modell eine klare Verbesserung.
