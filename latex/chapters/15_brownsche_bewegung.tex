...
Sei $(X_n)_{n \in \Bbb N}$ eine Folge von Zufallsvariablen. Wenn es eine Reihe $\sum_{n=1}^\infty a_n$ mit $a_n \geq 0$ gibt, so dass
...
Damit ist $\big(\hat W^{(n)}\big)_{n\in\Bbb N}$ fast sicher eine gleichmäßig Konvergent auf $[0,1]$ und konvergiert gleichmäßig 
gegen einen Grenzpfad $\widetilde W$, der als gleichmäßiger Grenzwert stückweise linearer Funktionen stetig ist. 
