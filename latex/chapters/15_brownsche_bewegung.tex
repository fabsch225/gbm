...
Induktiv folgt die Behauptung. $\hfill \square$ \\
...
\begin{lemma}[Reihenkriterium für fast sichere Konvergenz, Henze S. 201]
...
\begin{lemma}[Kolmogorov-Ungleichung, Behrends S. 70]
...
und es wird immer mehr Normalverteilungen in eine Zeiteinheit einbezogen. Vorerst ist jedoch unklar, 
...
\textit{Beweis.}
Fixiere $t \ge 0$ und setze $k:=\lfloor Nt \rfloor$ sowie $\alpha:=Nt-\lfloor Nt \rfloor \in [0,1)$. Dann gilt
$$
b^{(N)}(t)=\frac{1}{\sqrt{N}}\big(\xi_k+\alpha(\xi_{k+1}-\xi_k)\big)
=\frac{1}{\sqrt{N}}\big(\xi_k+\alpha\,\eta_{k+1}\big).
$$
Da $\xi_k \sim N(0,k)$ und $\eta_{k+1}\sim N(0,1)$ unabhängig sind, ist $b^{(N)}(t)$ normalverteilt mit Mittelwert $0$ und Varianz
$$
\sigma_N^2(t)=\frac{1}{N}\big(k+\alpha^2\big).
$$
Damit ist für jedes $x\in\Bbb R$ die Verteilungsfunktion von $b^{(N)}(t)$ gegeben durch
$$
...
$$
Dies ist die Verteilungsfunktion von $N(0,t)$. Definiere $W_t\sim N(0,t)$. Somit konvergiert für jedes feste $t$ die Verteilung von $b^{(N)}(t)$ gegen die von $W_t$. $\hfill\square$
\end{satz}

\begin{satz}[Stetigkeit der Pfade]
Der Pfad $t \mapsto W_t(\omega)$ ist fast sicher stetig.
\textit{Beweis.}
Fuer den Beweis wird eine neue Funktionen-Folge $\hat W_t^{(n)}(\omega), n \in \Bbb N$
...
\end{aligned}
$$
Für die unabhängigen, Inkremente $Z_{n,k}$, wobei
...
Damit ist $\big(\hat W^{(n)}\big)_{n\in\Bbb N}$ fast sicher eine Cauchy-Folge in $\Vert \cdot \Vert_{\infty}$ und somit
gleichmäßig Konvergent auf $[0,1]$. $\big(\hat W^{(n)}\big)_{n\in\Bbb N}$ konvergiert gleichmäßig 
gegen einen Grenzpfad $\widetilde W$, der als gleichmäßiger Grenzwert stückweise linearer  (also stetiger) Funktionen wiederum stetig ist. 
Durch Skalierung gilt die Aussage auf jedem kompakten Zeitintervall. $\hfill \square$
\end{satz}
...
Damit ist $W_t$ ein Martingal. $\hfill \square$
\end{satz}